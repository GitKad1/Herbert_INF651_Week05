...
function addTax (pizzaOrder) {
    const tax = Math.round((pizzaOrder[1] * 0.0925) * 100) / 100;
    const temp = pizzaOrder.pop();
    pizzaOrder.push(tax, temp);
    return pizzaOrder;
}
...
function isPalindrome (string1) {
    ...
    const forward = string1.split('');
    const backward = string1.split('').reverse();
